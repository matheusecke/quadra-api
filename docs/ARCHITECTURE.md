...
- `PrismaService` accepts either local `DATABASE_URL` or ECS-injected `DATABASE_SECRET` (RDS-managed JSON). Both resolve through `src/prisma/database-config.ts`; the Prisma CLI uses the same resolver via `prisma.config.ts`.
...
- `UsersModule` adds an org-scoped exact-email lookup (`GET /users/lookup`, `ORG_ADMIN`/`TEAM_ADMIN`) for invite flows. `TeamsModule` adds an org-admin affiliation-candidate search (`GET /teams/affiliation-candidates`) and narrows `PATCH /teams/:id` to the active own-team `TEAM_ADMIN` only — `SYSTEM_ADMIN` access was removed from that route and it has no `/admin` replacement yet. `OrganizationUserAffiliationsModule` gains a team-scoped member invite (`POST /teams/:teamId/organization-user-affiliations`) and explicit `POST /organization-user-affiliations/:id/activate`/`deactivate` lifecycle routes. `OrganizationTeamAffiliationsModule` gains `POST /organization-team-affiliations/:id/activate`/`deactivate` and its `POST /organization-team-affiliations` create route now performs composite onboarding (creates or reuses a team affiliation and invites its `TEAM_ADMIN` in one call). All previously existing routes are unchanged, including both `.../invite-response` endpoints, `PATCH organizations/:orgId/user-affiliations/:id/status`, `PATCH organizations/:orgId/team-affiliations/:id/status`, `GET /teams/:teamId/affiliations`, and the platform `POST /teams` / `PATCH /teams/:id/status` / `DELETE /teams/:id`. `OrganizationUserAffiliationsService` owns the shared `PENDING` user-invite transition (`createPendingInvite`, `transitionUserInvite`) used by direct user invites, team onboarding, and the B1 team-closing cascade on invite rejection/cancellation (see [DATABASE.md](./DATABASE.md)); `OrganizationTeamAffiliationsService` owns its own team-affiliation cascades (deactivate/activate propagation to member affiliations, onboarding cancellation); `AuthService` owns organization-context resolution (`findUsableOrgAffiliations`), which now also excludes team affiliations that are not `AffiliationStatus.ACTIVE`. No generic organization-management module was added — each module keeps its existing ownership boundary.
- Shared utilities:
